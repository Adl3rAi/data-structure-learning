...
int isSubLesser(bstNode *root, int val)
{
    if (root == NULL)
        return true;
    else if (root->data <= val && isSubLesser(root->left, val) && isSubLesser(root->right, val))
        return true;
    else
        return false;
}
int isSubGreater(bstNode *root, int val)
{
    if (root == NULL)
        return true;
    else if (root->data > val && isSubGreater(root->left, val) && isSubGreater(root->right, val))
        return true;
    else
        return false;
}
...
int main()
{
    ...
    // cout << getSuccessor(root, 4)->data << endl;
}